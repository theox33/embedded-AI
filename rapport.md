...
Table des formats que je confondais au début (et qui explique 80% des bugs visuels) :
...
### 3.1 Référence “pipeline computer vision” (contenu Step 1)

Au tout début, j’ai gardé cette checklist en tête (ça aide à comprendre où se place chaque étape dans le code) :
1. **Scene** : image / frame brute.
2. **Data Acquisition** : capture via caméra/senseur.
3. **Preprocessing** : resize, normalisation, réduction de bruit, conversions…
4. **Feature Extraction** : extraction de motifs (historiquement edges/corners/textures ; en deep learning c’est internalisé dans le réseau).
5. **Post-processing** : filtrage, NMS, segmentation/détection…
6. **Recognition & Understanding** : interprétation (classe/objet/action).
7. **Decision Making** : décision/action (ex. alerte, asservissement, robotique).
...
### 4.1 Où se passe quoi ? (table “anti-panique”)
...
Autre détail (repris du rapport Step 1) : l’initialisation est dans `Application.__init__` et le point d’entrée est le bloc `if __name__ == '__main__'`.
...
### 4.3 Difficultés rencontrées (Step 1)
...
Petite note “méthodo” : pour éviter de deviner, j’ai vérifié les dimensions entrée/sortie avec Netron (ou la spec fournie) avant d’écrire le post-process.
...
### 5.4.1 Détails “procédure Step 2” (sans rien omettre)

Le rapport Step 2 détaillait explicitement la procédure ; je la garde ici (réorganisée) :
...
### 5.6 Adaptations applicatives (repris du rapport Step 2)
...
### 5.7 Implications clés / attentes couvertes (repris du rapport Step 2)
...
### 5.8 Ce qui change vs Step 1 (repris du rapport Step 2)
...
### 5.9 Difficultés rencontrées (Step 2)
...
### 6.0 Objectifs et périmètre (repris du rapport Step 3)
...
Points qui doivent rester vrais (repris du rapport Step 3) :
...
Extrait “robustesse au nombre de classes” (repris du rapport Step 3) :
...
### 6.3 Difficultés rencontrées (Step 3)
...
### 6.4 Lanceur (commande utilisée, repris du rapport Step 3)
...
### 6.5 Résultat attendu vs implémenté (repris du rapport Step 3)
...
### 7.0 Objectifs et périmètre (repris du rapport Step 4)
...
### 7.0.1 Modèle / labels / lanceur (repris du rapport Step 4)
...
Création du tracker (version simple, sans paramètres — repris du rapport Step 4) :
...
Intégration “dans le flow” (appel juste après l’inférence, repris du rapport Step 4) :
...
### 7.4 Garde-fous (forme invalide / aucune détection) — repris du rapport Step 4

Le rapport Step 4 insiste sur deux garde-fous (sinon ça plante facilement sur une frame sans détection) :
...
### 7.5 Difficultés rencontrées (Step 4)
...
### 7.6 Overlay GTK : ce qui a été modifié (repris du rapport Step 4)
...
### 7.7 Lanceur (commande utilisée, reprise du rapport Step 4)
...
Astuce simple : si “trop de boîtes” → monter `conf_threshold`. Si “plus rien” → baisser `conf_threshold`.

## 9) Lancement (scripts fournis)

Chaque step a un lanceur :
- `./step1/launch_python_object_detection.sh`
- `./step2/launch_python_object_detection.sh`
- `./step3/launch_python_object_detection.sh`
...
Détail pratique (visible dans les scripts) : ils détectent parfois l’utilisateur `weston` et lancent la commande en conséquence (pour éviter les soucis d’affichage/permissions selon la session graphique).

## 10) Bilan perso (ce que j’ai appris)
...
- Factoriser le post-process YOLOv8 (Step 2/3/4) dans un module unique pour éviter les copies.
